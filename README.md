...
## Introduction
Whether it is Microcontrollers, FPGAs, or some ASIC they all have one thing in common and that is GPIO. GPIO stands for General Purpose Input Output. As the name suggests it can be used as both input and output depending on the state of control signals. GPIO is usually a subpart of a large system, primarily used for getting data from physical word or external devices or giving a command to external devices. This repository offers a GPIO design that works on 0.9V core and 3.3 I/O logic levels. All the communication or control signal logic is based on 0.9V design and the I/O functionality of GPIO operates at 3.3V logic.

## Software Used
The two major software without which this project was impossible is Synopsys Custom Compiler and Synopsys Primewave. A huge thanks to Synopsys India and IIT Hyderabad for providing the access to this elite software.
...
## CMOS Analysis
To know the behavior of a system we must know the behavior of the building blocks. The analysis and simulations are done to get the I-V characteristics of both 28nm NMOS and PMOS at the width of 0.1 um. 
...
#### NOR gate:
Like any simple CMOS-based nor gate this gate is made using two PMOS in series on the HIGH side and two NMOS in parallel on the low side. To cover the work case scenario when the output is to be pulled high, as the two PMOS resistance add up, the width of PMOS taken is double the width of PMOS in a balanced CMOS circuit.
...
#### NAND gate:
Unlike NOR in NAND, we have two PMOS in parallel on the HIGH side and two NMOS in series on the LOW side. In the worst-case scenario when the output is to be pulled low, due to the series connection of NMOS the resistance adds up so to compensate for this the NMOS width should be double the NMOS width of a standard CMOS inverter.
...
As the name suggests Level Shifter is used to convert the signal of one logic level to a different logic level, in this case, we are converting a 0.9V logic to 3.3V logic. This device is used to convert the input signal of 0.9V to 3.3V so that the CMOS working at 3.3V can be operated properly.
...
#### Schmitt Trigger:
Schmitt Trigger plays an important role in converting the 3.3V signal arriving at the I/0 port into the 0.9V logic level which can be transmitted further. Schmitt Trigger has two important voltage thresholds Vth+ and Vth-. When the input signal is above Vth+ the Schmitt Trigger outputs a logic high and when the input signal is below Vth- the Schmitt Trigger outputs logic low. If the signal is between Vth+ and Vth- then the output of Schmitt Trigger does not change its state.
...
One of the most crucial sub-circuit of the GPIO is the MOSFET driver. This drive takes two inputs namely "EN" and "DATA IN". "EN" stands for enabling and when this pin is high the output of the CMOS driven by this driver is either low or high depending on the data input value. If the EN pin is LOW then both PMOS and NMOS of CMOS are inactive this allows other circuitry to perform operations on the I/O port. Last but not the least, this drive has the feature of dead time in between the transition from low to high. The dead time between OUTPUT HIGH and OUTPUT LOW while going from high to low is 5ns and 10ns while going from low to high.
...
The above figure represents the schematic of GPIO. The GPIO mainly consists of three sections namely INPUT, OUTPUT, and INPUT PULL UP/DOWN. The ESD diodes are a special arrangement of NMOS and the drop of ESP diode is nearly around 0.7V. The control signals are:
...
INPUT is enabled or disabled by the INPUT_EN (logic high) pin. This sub-circuit of the GPIO consists of one AND gate and a Schmitt Trigger. Here the Schmitt Trigger gets whatever is on the I/O port and is between 3.3V to 0V, it then converts the signal to 0.9V logic. Schmitt Trigger is used because it rejects a band of voltage range that might otherwise have changed the output.

### OUTPUT

Going with the flow OUTPUT is enabled or disabled by the OUTPUT_EN (active high) control signal. The circuit at the GPIO level consists of CMOS with high current capability and a MOSFET driver to drive the Gate of these MOSFETS. The drive inbuilt RC-based dead time circuit. As the MOSFETS driven by this driver work in the mA range the current spike during the transition from low to high may affect the circuit performance. To eliminate this the PMOS it turned 5ns after the NMOS is turned off and when switching from HIGH to LOW the PMOS is turned OFF 10ns before the NMOS turns on.
...
To provide a different combination of the control signal, output data, and PU/PD states we are using the pulse generator from the analogLib. All the pulse sources are operating on a 50% duty cycle. To generate an analog signal at the output PAD a sine wave source is connected with the PAD by the means of 1M Ohm, this big resistance allows us to simulate all I/O features at the same time. The description of the time period and frequency of different pulses are as follows:
...
## Conclusion
This repository presents the design and simulation of 0.9V-3.3V GPIO on a 28nm technology node and offers a reference for basic GPIO designing. Future works can include the addition of INTERRUPT, ANALOG INPUT, and PIN CAPTURE.
## Author
